...
class TestSessionManagement(unittest.TestCase):
    ...
    def setUp(self):
        ...
        server.active_session["public"] = {
            "token": None, "expires_at": None, "ip_address": None}
        server.active_session["admin"] = {
            "token": None, "expires_at": None, "ip_address": None}
    ...
    def test_create_session_sets_token(self):
        ...
        with patch("server.datetime") as mock_datetime:
            ...
            token = server.create_session(client_ip, "public")

            self.assertIsNotNone(server.active_session["public"]["token"])
            self.assertEqual(server.active_session["public"]["token"], token)
            self.assertEqual(
                server.active_session["public"]["ip_address"], client_ip)
            self.assertEqual(
                server.active_session["public"]["expires_at"], now + timedelta(minutes=60))
    ...
    def test_is_session_valid_returns_false_for_wrong_token(self):
        ...
        server.active_session["public"]["token"] = "correct_token"
        server.active_session["public"]["ip_address"] = "192.168.1.100"
        server.active_session["public"]["expires_at"] = datetime.now() + \
            timedelta(minutes=30)

        result = server.is_session_valid(
            "wrong_token", "192.168.1.100", "public")
        self.assertFalse(result)
    ...
    def test_is_session_valid_returns_false_for_wrong_ip(self):
        ...
        token = "valid_token"
        server.active_session["public"]["token"] = token
        server.active_session["public"]["ip_address"] = "192.168.1.100"
        server.active_session["public"]["expires_at"] = datetime.now() + \
            timedelta(minutes=30)

        result = server.is_session_valid(token, "192.168.1.200", "public")
        self.assertFalse(result)
    ...
    def test_is_session_valid_returns_false_when_expired(self):
        ...
        with patch("server.datetime") as mock_datetime:
            ...
            server.active_session["public"]["expires_at"] = past_time
            mock_datetime.datetime.now.return_value = current_time

            result = server.is_session_valid(token, client_ip, "public")
            self.assertFalse(result)
            self.assertIsNone(server.active_session["public"]["token"])
    ...
    def test_is_session_valid_returns_true_for_valid_session(self):
        ...
        with patch("server.datetime") as mock_datetime:
            ...
            server.active_session["public"]["expires_at"] = future_time
            mock_datetime.datetime.now.return_value = current_time
...
class TestUtilityHelpers(unittest.TestCase):
    """Test helper utilities used by request handling."""
    ...
    def test_extract_session_token_returns_value_from_cookie_header(self):
        """Test that session token is extracted from a cookie header."""
        cookie_header = "foo=bar; session_token=test123; another=value"
        token = server._extract_session_token(cookie_header)
        self.assertEqual(token, "test123")

    def test_build_health_payload_reports_session_states(self):
        """Test that health payload reflects public/admin session activity."""
        server.active_session["public"]["token"] = "public-token"
        payload = json.loads(server._build_health_payload().decode("utf-8"))
        self.assertEqual(payload["status"], "ok")
        self.assertEqual(payload["public_session"], "active")
        self.assertEqual(payload["admin_session"], "none")
